...
def parse_args():
    ...
    parser.add_argument(
        "--for-pareto", action="store_true", help="wheter rpfs for pareto"
    )
...
if __name__ == "__main__":
    ...
    export_directory = f"data/rpfs_ex/{args.l}/{args.d}"
    file_id = str(uuid.uuid4())
    file_name = file_id
    if args.for_pareto:
        file_name = f"for-pareto_{file_id}"
    export_path = f"{export_directory}/{file_name}.pkl"
