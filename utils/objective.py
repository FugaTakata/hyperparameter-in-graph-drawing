...
def ss_objective(
    ...
    def objective(trial: optuna.Trial):
        ...
        for q in all_quality_metrics_names:
            quality_metrics_means[q] = []

        for _ in range(N):
            pos = sgd(graph, indices, params, random.randint(0, 2**32))
            ...
            for q in quality_metrics:
                quality_metrics_means[q].append(quality_metrics[q])
        for q in all_quality_metrics_names:
            quality_metrics_means[q] = statistics.mean(
                quality_metrics_means[q]
            )

        trial.set_user_attr("quality_metrics", quality_metrics_means)
