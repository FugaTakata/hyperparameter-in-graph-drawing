...
ls = [SS, FR]
ds = [
    {"from": "les_miserables", "to": "1138_bus"},
    {"from": "les_miserables", "to": "USpowerGrid"},
    {"from": "1138_bus", "to": "USpowerGrid"},
]
...
export_directory = f"data/question3/{args.l}/{question_id}"
...
for d in ds:
    dataset_path = f"lib/egraph-rs/js/dataset/{d['to']}.json"
    with open(dataset_path) as f:
        graph_data = json.load(f)
    nx_graph = graph_preprocessing(nx.node_link_graph(graph_data), EDGE_WEIGHT)
    storage_path = f"sqlite:///db/opt/{args.l}/{d['from']}/{study_name}.db"
    ...
    for qi in range(1, N_Q + 1):
        ...
        images_export_directory = (
            f"{export_directory}/images/{d['from']}-{d['to']}/{qi}"
        )
        ...
        os.makedirs(f"{export_directory}/{d['from']}-{d['to']}", exist_ok=True)
        question_image_export_path = (
            f"{export_directory}/{d['from']}-{d['to']}/{qi}.png"
        )
        ...
        # with open(
        #     f"{export_directory}/{d['from']}-{d['to']}/{qi}/info.json",
        #     mode="w",
        # ) as f:
        #     json.dump(info, f)
