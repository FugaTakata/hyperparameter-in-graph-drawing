function g_ex3() {
  ...
  const FR = "FR";
  const imageUrls = {};
  const rootFolderId = "1CkEn-rqM6d_hZMsgvTF9KStb_KQi7eGC";
  ...
  for (let i = 0; i < keys[FR].length; ++i) {
    ...
    const formTitle = `評価実験その2-${i + 1}`;
    console.log(imageUrls);
    console.log(imageUrls["FR"]);
    createEx3Form(formTitle, imageUrls, questionIds);
  }
}

function createEx3Form(formTitle, imageUrls, questionIds) {
  const formDescription = `実験へのご協力ありがとうございます。
実験その1をまだやっていない人は、実験その1に先に取り組んでください。

実験その1が終わっている人は下記の説明を読んだ上で実験に進んでください。
...
例えばノードの配置やエッジの明瞭さなどの一般的な基準をもとにしたものでも、芸術的な美しさや「パット見いい感じ」みたいな好ましさであっても構いません。`;
  ...
  for (const l of ls) {
    for (const p of ps) {
      ...
      pageBreak
        .setTitle(`${p}パターンを${drawingNameMap[l]}で描画した結果の評価`)
        .setHelpText(
          "リンクから描画結果を確認して、好ましいと感じる描画結果の番号を4個選択してください。"
        );
      for (const qId of qIds) {
        const item = form.addCheckboxItem();
        item.setTitle(`${l}:${questionIds[l]}:${p}:${qId}`);
        item.setHelpText(`${imageUrls[l][questionIds[l]][p][qId]}`);
